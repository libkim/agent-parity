...
sed "s/^PACKAGED_VERSION=\"dev\"$/PACKAGED_VERSION=\"$VERSION\"/" bootstrap/install.sh > "$OUT/install.sh"
sed "s/^\\\$PackagedVersion = \"dev\"$/\\\$PackagedVersion = \"$VERSION\"/" bootstrap/install.ps1 > "$OUT/install.ps1"
sed "s/^PACKAGED_VERSION=\"dev\"$/PACKAGED_VERSION=\"$VERSION\"/" bootstrap/update.sh > "$OUT/update.sh"
sed "s/^\\\$PackagedVersion = \"dev\"$/\\\$PackagedVersion = \"$VERSION\"/" bootstrap/update.ps1 > "$OUT/update.ps1"
