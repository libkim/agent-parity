...
agent-parity는 이 둘을 저장소 안의 평문 파일로 만듭니다. 커밋해두면 소스와 함께 버전 관리되는 **코드형 환경**이 됩니다. 메모리가 저장소에 담기니 새 머신에 클론하거나 팀원에게 넘기면 맥락이 따라오고, 받는 쪽은 설치 명령 없이 다음 세션부터 바로 씁니다. 함께 설치되는 merge 드라이버가 두 머신에서 동시에 편집한 내용도 합쳐줍니다. 스킬과 지침은 Claude Code·Codex·Cursor·Antigravity가 모두 읽는 하나의 공유 정의가 되어, 모든 에이전트에서 일관되게 유지됩니다.
