...
코딩 에이전트는 메모리·스킬·지침을 저마다 따로, 대개 저장소 밖 머신 로컬에 둡니다. 그래서 그 맥락은 다른 머신이나 팀원에게 따라오지 않고, 에이전트를 바꾸면 매번 다시 설정해야 합니다.

agent-parity는 이 셋을 저장소에 커밋하는 **코드형 환경**으로 만들어 이 문제를 해결합니다. 저장소 안에 함께 담겨 있으니, 새 머신에 클론하거나 팀원에게 넘기면 그 맥락까지 그대로 옮겨지고, 받는 쪽은 설치 명령 없이 다음 세션부터 바로 사용합니다. 여러 에이전트를 함께 쓰면 Claude Code·Codex·Cursor·Antigravity가 같은 메모리를 공유하고 같은 스킬과 지침을 읽습니다.
