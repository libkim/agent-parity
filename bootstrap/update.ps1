# agent-parity: native Windows updater.
[CmdletBinding()]
param(
...
$script:ConfigIssues = 0

function Add-ConfigIssue([string]$File, [string]$Detail) {
  $script:ConfigIssues++
  Write-Warning "$File -- $Detail; edit this file manually"
}

function Invoke-ConfigEditor([Parameter(ValueFromRemainingArguments = $true)] [string[]]$EditorArgs) {
  $previousPreference = $ErrorActionPreference
  try {
    $ErrorActionPreference = "Continue"
    $output = @(& $ConfigEditor @EditorArgs 2>&1)
    $code = $LASTEXITCODE
  } finally {
...
function Usage {
  Write-Error "usage: update.ps1 [update] [dir]"
  exit 2
}
...
function Match-Newlines([string]$Text, [string]$ReferencePath) {
  if (!(Test-Path -LiteralPath $ReferencePath -PathType Leaf)) { return $Text }
  $existing = [System.IO.File]::ReadAllText($ReferencePath, [System.Text.Encoding]::UTF8)
  $newline = if ($existing.Contains("`r`n")) { "`r`n" } elseif ($existing.Contains("`n")) { "`n" } else { $null }
...
function Install-ProjectCli {
  ...
  Write-Text (Join-Path $d $cmdName) ((Fetch-Text "templates/project-agent-parity.cmd").TrimEnd("`r", "`n") + "`r`n")
...
function Install-Server {
  ...
  try {
    ...
    foreach ($entry in @(@("VERSION", $Version + "`n"), @("RELEASE", $Release.TrimEnd('/') + "`n"))) {
      $name = $entry[0]
      $final = Join-Path $dest $name
      $content = Match-Newlines $entry[1] $final
      Write-Text (Join-Path $stage $name) $content
    }
...
function Reg-McpConfig([string]$Rel, [string]$Template, [string]$Marker) {
  ...
  if ($null -eq $existing) {
    ...
    $call = Invoke-ConfigEditor command $t
    $current = $call.Output
    $code = $call.ExitCode
    if ($code -eq 0) {
      ...
      if ($currentText -in @(".agents/mcp/memory/run.sh", ".agents/mcp/memory/run.cmd")) {
        ...
        if ($call.ExitCode -ne 0) {
          Add-ConfigIssue $Rel (($result | Out-String).Trim())
        } elseif (($result | Out-String).Trim() -eq "changed") {
          Write-Output "  updated:    $Rel (managed registration and approvals)"
        } else {
          Write-Output "  registered: $Rel (already)"
        }
      } else {
        Add-ConfigIssue $Rel "memory entry points at a different server"
      ...
      Add-ConfigIssue $Rel (($current | Out-String).Trim())
      $c -split "`n" | ForEach-Object { Write-Output "    | $_" }
    }
...
function Reg-CursorCli {
  ...
  $call = Invoke-ConfigEditor merge-cursor-cli $t
  $result = $call.Output
  if ($call.ExitCode -ne 0) { Add-ConfigIssue $CursorCli (($result | Out-String).Trim()); return }
...
function Reg-AgentHooks {
  ...
  foreach ($entry in @(
    ...
    $kind = $entry.Kind
    $path = $entry.Path
    $call = Invoke-ConfigEditor merge-hook (Path-InTarget $path) $kind
    $result = $call.Output
    if ($call.ExitCode -ne 0) {
      Add-ConfigIssue $path (($result | Out-String).Trim())
    }
  }
  if ($before -eq $script:ConfigIssues) { Write-Output "  hooks:      Claude, Codex, Cursor, Antigravity self-heal registered" }
  else { Write-Output "  hooks:      completed with configuration warnings" }
...
function Test-DirectorySame {
  ...
  $leftItems = @(Get-ChildItem -LiteralPath $leftRoot -Recurse -Force | ForEach-Object {
    $relative = $_.FullName.Substring($leftRoot.Length).TrimStart('\', '/')
    if ($_.PSIsContainer) { "D|$relative" } else { "F|$relative|$((Get-FileHash -LiteralPath $_.FullName -Algorithm SHA256).Hash)" }
  })
  $rightItems = @(Get-ChildItem -LiteralPath $rightRoot -Recurse -Force | ForEach-Object {
    $relative = $_.FullName.Substring($rightRoot.Length).TrimStart('\', '/')
    if ($_.PSIsContainer) { "D|$relative" } else { "F|$relative|$((Get-FileHash -LiteralPath $_.FullName -Algorithm SHA256).Hash)" }
  })
  if ($leftItems.Count -ne $rightItems.Count) { return $false }
  return (Compare-Object ($leftItems | Sort-Object) ($rightItems | Sort-Object)).Count -eq 0
}
...
function Adopt-AgentSkills {
  ...
  foreach ($pair in @(@(".claude/skills", "claude"), @(".codex/skills", "codex"), @(".cursor/skills", "cursor"))) {
    ...
    Get-ChildItem -LiteralPath $src -Directory | ForEach-Object {
      ...
      } elseif (!(Test-DirectorySame $_.FullName $dest)) {
        $conflict = Path-InTarget ".agents/skills/$name.from-$label"
        if (Test-Path -LiteralPath $conflict) {
          Write-Warning "$dir/$name differs from both .agents/skills/$name and the existing .agents/skills/$name.from-$label; left it in place for manual review"
        } else {
...
function Install-Skills {
  ...
  $call = Invoke-ConfigEditor merge-claude-settings $src $ClaudeHook
  $mergeResult = $call.Output
  if ($call.ExitCode -eq 0) {
    Write-Output "  merged:     $ClaudeSrc (memory keys + sync hook)"
    $syncResult = & powershell -NoProfile -ExecutionPolicy Bypass -File $s sync 2>&1
    if ($LASTEXITCODE -eq 0) { $syncResult | ForEach-Object { Write-Output "  $_" } }
    else { Add-ConfigIssue $ClaudeTgt (($syncResult | Out-String).Trim()) }
  } else {
    Add-ConfigIssue $ClaudeSrc (($mergeResult | Out-String).Trim())
  }
}
...
function Sync-AgentsBlock {
  ...
  if ($state -eq "valid") {
    ...
    if ($next -eq $text) { Write-Output "AGENTS.md: agent-parity instruction block up to date" }
    else { Write-Text $ag $next; Write-Output "AGENTS.md: refreshed agent-parity instruction block" }
    ...
    Write-Output "AGENTS.md: appended agent-parity instruction block"
  } else {
...
function Cmd-Update {
  ...
  Clear-VersionCache
  Warn-Parity
  if ($script:ConfigIssues -gt 0) { Write-Output "configuration: completed with $script:ConfigIssues warning(s); listed files require manual review" }
  else { Write-Output "configuration: all managed agent settings converged" }
