...
try {
  ...
  $installedVersion = [IO.File]::ReadAllText((Join-Path $testRoot ".agent-parity\mcp\memory\VERSION")).Trim()
  ...
  $statusPath = Join-Path $testRoot ".agent-parity\scripts\status.ps1"
  ...
  $crlfFiles = @(
    ".agent-parity\scripts\common.ps1",
  ...
  [IO.File]::WriteAllText((Join-Path $testRoot ".agent-parity\mcp\memory\VERSION"), "v0.0.0-old`r`n", (New-Object Text.UTF8Encoding($false)))
  ...
  foreach ($relative in @($crlfFiles + ".agent-parity\mcp\memory\VERSION")) {
  ...
  [IO.File]::WriteAllText($codexPath, "[mcp_servers.memory]`ncommand = `".agent-parity/mcp/memory/run.cmd`"`ndefault_tools_approval_mode = `"prompt`"`n")
  Remove-Item -LiteralPath (Join-Path $testRoot ".agents\hooks.json") -Force
  [IO.File]::WriteAllText((Join-Path $testRoot ".agent-parity\mcp\memory\VERSION"), "v0.0.0-old`n")
  ...
  & (Join-Path $testRoot ".agent-parity\bin\agent-parity.cmd") uninstall
  if ($LASTEXITCODE -ne 0) { throw "offline Windows uninstall failed with $LASTEXITCODE" }
  if (Test-Path -LiteralPath (Join-Path $testRoot ".agent-parity\mcp\memory")) { throw "uninstall left the project memory server directory" }
