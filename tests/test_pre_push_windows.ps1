...
# The PowerShell install path (Reg-PrePushHook) owns .git/hooks/pre-push as a
# dispatcher: it writes the managed hook in a git repo, preserves a pre-existing
# user hook as pre-push.user and chains it, restores it on uninstall, and defers
# entirely when a hook manager owns the dir through core.hooksPath. The unix
# test (test_pre_push.sh) covers the shell installer; this covers the ps1 one.
...
function Invoke-Install {
  ...
  try {
    Invoke-Expression ([IO.File]::ReadAllText((Join-Path $dist "install.ps1"), [Text.Encoding]::UTF8)) *> $null
...
try {
  # 1) Fresh git repo, no prior hook: install writes the dispatcher with our marker.
  $r1 = New-TestRepo; $roots += $r1
  Invoke-Install -Root $r1
  $hook1 = Join-Path $r1 ".git\hooks\pre-push"
  if (!(Test-Path -LiteralPath $hook1 -PathType Leaf)) { throw "install did not write the pre-push dispatcher" }
  if (![IO.File]::ReadAllText($hook1).Contains($marker)) { throw "dispatcher missing the managed marker" }

  # 2) Pre-existing user hook: install preserves it as pre-push.user and chains it.
  ...
  [IO.File]::WriteAllText($hook2, "#!/bin/sh`ncat >/dev/null`necho MINE-RAN`n")
  Invoke-Install -Root $r2
  if (![IO.File]::ReadAllText($hook2).Contains($marker)) { throw "install did not install the dispatcher over a user hook" }
  if (!(Test-Path -LiteralPath "$hook2.user" -PathType Leaf)) { throw "install did not preserve the user hook as pre-push.user" }
  if (![IO.File]::ReadAllText("$hook2.user").Contains("MINE-RAN")) { throw "pre-push.user is not the original user hook" }
  # uninstall restores the user's original hook and removes pre-push.user.
  & (Join-Path $r2 ".agents\bin\agent-parity.cmd") uninstall *> $null
  if (![IO.File]::ReadAllText($hook2).Contains("MINE-RAN")) { throw "uninstall did not restore the user's original pre-push hook" }
  if (Test-Path -LiteralPath "$hook2.user") { throw "uninstall left pre-push.user behind" }
  ...
  Write-Output "pre-push (PowerShell path): dispatcher install, user-hook chain + restore, core.hooksPath deferral: OK"
} finally {
