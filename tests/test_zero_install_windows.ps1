...
try {
  ...
  New-Item -ItemType Directory -Force -Path "$root\.agent-parity\scripts", "$root\.agent-parity\mcp\memory", "$root\.agents", "$root\.cursor", "$root\.codex" | Out-Null
  Copy-Item -LiteralPath (Join-Path $testRepoRoot "templates\common.ps1"),(Join-Path $testRepoRoot "templates\self-heal.ps1") -Destination "$root\.agent-parity\scripts"
  Copy-Item -LiteralPath (Join-Path $testRepoRoot "templates\run.cmd") -Destination "$root\.agent-parity\mcp\memory"
  [IO.File]::WriteAllText("$root\.agent-parity\mcp\memory\VERSION", "$Version`n")
  [IO.File]::WriteAllText("$root\.agent-parity\mcp\memory\RELEASE", "$base/dist`n")
  $json = '{"mcpServers":{"memory":{"command":".agent-parity/mcp/memory/run.sh"}}}'
  ...
  $output = @(& "$root\.agent-parity\scripts\self-heal.ps1")
  ...
  $second = @(& "$root\.agent-parity\scripts\self-heal.ps1")
